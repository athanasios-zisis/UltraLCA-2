...
def _cross_check_messages(result: DatasetRunResult) -> list[str]:
    if len(result.ultrabubble_runs) < 2:
        return []

    runs_by_method = {item.method: item for item in result.ultrabubble_runs}
    if "naive" in runs_by_method:
        if len(runs_by_method) == 1:
            return []

        naive_signature = tuple(runs_by_method["naive"].ultrabubbles)
        mismatched_labels = [
            METHOD_LABELS[item.method]
            for item in result.ultrabubble_runs
            if item.method != "naive" and tuple(item.ultrabubbles) != naive_signature
        ]
        if not mismatched_labels:
            return ["Cross-check: all selected methods match naive."]

        mismatched_text = ", ".join(mismatched_labels)
        verb = "does" if len(mismatched_labels) == 1 else "do"
        return [f"Warning: {mismatched_text} {verb} not match naive."]

    groups: dict[tuple[Snarl, ...], list[str]] = {}
    for item in result.ultrabubble_runs:
        signature = tuple(item.ultrabubbles)
        groups.setdefault(signature, []).append(METHOD_LABELS[item.method])

    if len(groups) == 1:
        return ["Cross-check: all selected methods match."]

    mismatch_groups = " | ".join(", ".join(labels) for labels in groups.values())
    return [
        "Warning: selected methods do not all match.",
        f"Mismatch groups: {mismatch_groups}",
    ]
...
def _print_dataset_result(result: DatasetRunResult, show_ultrabubbles: bool) -> None:
    ...
    cross_check_messages = _cross_check_messages(result)
    if cross_check_messages:
        print()
        for message in cross_check_messages:
            print(f"  {message}")
